...
| 操作 | 説明 |
|---|---|
| `!stop` / `!cancel` / `!interrupt` | スレッドにこれを送ると走行中の claude を即 kill |
| **✏️ その他（自由入力）** ボタン | AskUserQuestion の選択肢に必ず追加される。押した後にスレッドに送るメッセージが回答になる |
| 走行中にメッセージ送信 | 📥 リアクション付与 + 「N 件待機中」表示。現ターン完了後に順番に処理 |
